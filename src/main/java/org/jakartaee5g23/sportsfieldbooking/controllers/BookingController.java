...
public class BookingController {
    ...
    public ResponseEntity<BookingResponse> refund(@PathVariable String id) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(bookingMapper.toBookingResponse(bookingService.updateStatus(id, REFUND_REQUESTED)));
    }
    ...
    public ResponseEntity<PaginateResponse<BookingResponse>> getHistory(@RequestParam(defaultValue = "0") String offset,
        ...
        Page<Booking> bookings = bookingService.findBookingHistoryByUser(current, statuses, Integer.parseInt(offset),
                Integer.parseInt(limit));
        return ResponseEntity.status(HttpStatus.OK)
                .body(PaginateResponse.<BookingResponse>builder()
                        .items(bookings.stream().map(bookingMapper::toBookingResponse).toList())
    ...
    public ResponseEntity<PaginateResponse<BookingResponse>> getUpcomingBookings(
            @RequestParam(defaultValue = "0") String offset,
            @RequestParam(defaultValue = "100") String limit) {
        Page<Booking> bookings = bookingService.getUpcomingBookings(getUserIdFromContext(), Integer.parseInt(offset),
                Integer.parseInt(limit));
        return ResponseEntity.status(HttpStatus.OK)
                .body(PaginateResponse.<BookingResponse>builder()
                        .items(bookings.stream().map(bookingMapper::toBookingResponse).toList())
